...
public:
    ...
    /// Set the target position. The run() function will try to run the motor (at most one step per call)
...
private:
    ...
    /// The target position in steps. The AccelStepper library will run the
...
/// The stepper will run at a constant speed to each newly set posiiton,
